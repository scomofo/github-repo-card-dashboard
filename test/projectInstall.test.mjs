...
import { buildLauncher, createProjectInstaller, findManagerPath, inspectProject, windowsQuote } from '../src/projectInstall.mjs';
...
test('package manager lookup is PATHEXT-aware on Windows', async (t) => {
  const temp = await mkdtemp(path.join(tmpdir(), 'repo-dashboard-pathtest-'));
  t.after(() => rm(temp, { recursive: true, force: true }));
  const bin = path.join(temp, 'bin');
  await mkdir(bin);
  await writeFile(path.join(bin, 'npm.cmd'), '@echo off\r\necho npm-shim\r\n');
  await chmod(path.join(bin, 'npm.cmd'), 0o700);
  const env = { PATH: bin };
  assert.equal(await findManagerPath('npm', { env, platform: 'win32' }), path.join(bin, 'npm.cmd'));
  assert.equal(await findManagerPath('pnpm', { env, platform: 'win32' }), null);
  // On POSIX the .cmd shim alone is not a match; only the bare executable is.
  assert.equal(await findManagerPath('npm', { env, platform: 'linux' }), null);
  await writeFile(path.join(bin, 'npm'), `#!${process.execPath}\n`);
  await chmod(path.join(bin, 'npm'), 0o700);
  assert.equal(await findManagerPath('npm', { env, platform: 'linux' }), path.join(bin, 'npm'));
  // A bare executable is still usable on Windows when no shim exists.
  await rm(path.join(bin, 'npm.cmd'));
  assert.equal(await findManagerPath('npm', { env, platform: 'win32' }), path.join(bin, 'npm'));
});

test('Windows argv quoting follows CommandLineToArgvW rules', () => {
  assert.equal(windowsQuote('simple'), 'simple');
  assert.equal(windowsQuote('C:\\Program Files\\app'), '"C:\\Program Files\\app"');
  assert.equal(windowsQuote('say "hi"'), '"say \\"hi\\""');
  assert.equal(windowsQuote('C:\\my dir\\'), '"C:\\my dir\\\\"');
  assert.equal(windowsQuote('C:\\dir\\'), 'C:\\dir\\');
});

test('generated Windows launcher is a batch file that keeps the marker and quoted paths', () => {
  const script = buildLauncher({ platform: 'win32', nodePath: 'C:\\Program Files\\nodejs\\node.exe',
    scriptPath: 'C:\\dash\\scripts\\project-launcher.mjs', recordPath: 'C:\\Users\\me\\AppData\\Roaming\\Repo Dashboard Projects\\owner\\demo.json' });
  assert.match(script, /^@echo off\r\n/);
  assert.ok(script.includes('# Generated by Repo Dashboard.'), 'the describe() marker survives in a batch comment');
  assert.ok(script.includes('"C:\\Program Files\\nodejs\\node.exe"'), 'paths with spaces are quoted');
  assert.ok(script.includes('C:\\dash\\scripts\\project-launcher.mjs'), 'paths without spaces stay bare');
  const posix = buildLauncher({ platform: 'darwin', nodePath: '/usr/local/bin/node',
    scriptPath: '/dash/scripts/project-launcher.mjs', recordPath: '/state/owner/demo.json' });
  assert.match(posix, /^#!\/bin\/bash\n/);
  assert.ok(posix.includes('# Generated by Repo Dashboard.'));
});

test('install on a Windows platform writes a .bat launcher', async (t) => {
  const f = await fixture(t, null);
  await writeFile(path.join(f.directory, 'index.html'), '<html><body>hi</body></html>');
  const installer = createProjectInstaller({ stateRoot: f.stateRoot, launchersRoot: f.launchersRoot, env: f.env, platform: 'win32' });
  const result = await installer.install(f.input);
  assert.equal(result.ready, true);
  assert.ok(result.launcherPath.endsWith('.bat'), result.launcherPath);
  const content = await readFile(result.launcherPath, 'utf8');
  assert.ok(content.includes('# Generated by Repo Dashboard.'));
